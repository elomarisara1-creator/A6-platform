...
// Cache content in memory for 30 minutes
const cache = {};
...
async function listFiles(folderId, token) {
  const res = await fetch(
    `https://www.googleapis.com/drive/v3/files?q='${folderId}'+in+parents+and+trashed=false&fields=files(id,name,mimeType)&pageSize=20`,
...
async function getFileText(file, token) {
  try {
    ...
    // Limit each file to 8000 chars for speed
